...
工程上很多选择不是“最好”和“最差”的二选一，而是“当前能稳定推进”和“理论上更漂亮但还没验证”的取舍。微信端如果现在强行押 Wechaty，就必须先解决 puppet、登录、稳定性、风控、部署成本这些问题。每一个都不是不能做，但都会把主线拖进不确定性里。

而 Claw / WxClaw 的好处是可以快速建立一条可用链路。它不完美，但它能让我们先验证后端能力：消息能不能进来，NoneBot 插件能不能复用，LLM 记忆是否合理，工具调用是否可控。这些才是 Bot 的核心资产。

换句话说，微信端现在用 Claw，不是因为它是最优解，而是因为它是当前最省心的可运行解。这是无奈之举，但不是乱来。
